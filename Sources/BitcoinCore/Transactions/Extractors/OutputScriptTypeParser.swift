...
class OutputScriptTypeParser: ITransactionExtractor {
    ...
    func parseScriptType(output: Output) {
        ...
        if
            lockingScriptCount == ScriptType.p2pkh.size,
            // P2PKH Output script 25 bytes: 76 A9 14 {20-byte-key-hash} 88 AC
            ...
            lockingScriptCount == ScriptType.p2pk.size || lockingScriptCount == 67,
            // P2PK Output script 35/67 bytes: {push-length-byte 33/65} {length-byte-public-key 33/65} AC
            ...
            lockingScriptCount == ScriptType.p2wpkh.size,
            // P2WPKH Output script 22 bytes: {version-byte {00} 14 {20-byte-key-hash}
            ...
            lockingScriptCount == ScriptType.p2tr.size,
            // P2TR Output script 34 bytes: {version-byte 51} {51} 20 {32-byte-public-key}
